...
const ConversationScreen = ({
  ...
  const showMessages = () => {
    //nếu giao diện người dùng đang tải thông báo ở hậu trường, hiển thị thông báo được truy xuất từ ​​SSR Tiếp theo (được chuyển xuống từ [id] .tsx)
    if (messagesLoading) {
      return messages.map((message) => (
        <Message key={message.id} message={message} />
      ));
    }
    //nếu giao diện người dùng đã tải xong tin nhắn, vì vậy bây giờ chúng ta có messageSnapshot
    if (messagesSnapshot) {
      transformMessage;
